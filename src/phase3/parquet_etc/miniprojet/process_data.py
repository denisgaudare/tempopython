...
print(df.group_by("origin").count())

print("\n📊 Durée moyenne des vols par compagnie :")
print(df.group_by("airline").agg(pl.col("duration_minutes").mean().alias("avg_duration")))
...
print(df.group_by("departure_time").count().sort("departure_time"))
